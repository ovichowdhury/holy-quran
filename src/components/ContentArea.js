...
class ContentArea extends Component {
  ...
  render() {
    const showSurah = (
      ...
      <div className="row d-flex justify-content-center mt-3 " style={{ cursor: "pointer" }}>
        {
          this.state.surahList.map(s => (
            ...
            <div className="col-sm-4 mt-3" key={s.id} >
              <div className="card" style={{ boxShadow: "1px 2px 3px rgba(0, 0, 0, .1)" }}>
                <div className="card-body m-auto" onClick={() => this.handleClick(s.id)} >
                  <p style={{ fontSize: "20px" }}>
                    <span>{s.id}.</span> {s.surahNameEng} <span className="text-success">( {s.surahNameArb} )</span>{" "}
                    {/* <span className="text-info" style={{ fontSize: "10px" }}>{s.englishTitle}</span> */}
                  </p>
                  <small>Details: {s.details} , Total Ayat: <span className="text-warning">{s.totalAyat}</span></small>
                </div>
              </div>
            </div>
        ...
        <div className="row d-flex justify-content-center mt-4">
          <div className="radio mr-3">
            <label className="text-success" style={{ cursor: "pointer" }}>
              <input type="radio" name={this.state.surah} onClick={this.onSurah} checked={this.state.checkedButton} />&nbsp;
              Surah
            </label>
          </div>
          <div className="radio">
            <label className="text-warning" style={{ cursor: "pointer" }}>
              <input type="radio" name={this.state.para} onClick={this.onpara} checked={!this.state.checkedButton} />&nbsp;
              Para
           </label>
          </div>
        </div>
